...
<g:if test="${flash.result?.r}">
	<!-- 预览与撤销 -->
	<ui:form action="undo">
		<input type="hidden" name="domain" value="classGrade"/>
		...
		<div id="modal-review" class="modal hide fade" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			...
			<ui:table class="table-bordered table-hover table-condensed">
				<thead>
				<ui:tr>
					<ui:th text="classGrade.name.label"/>
					<ui:th text="classGrade.grade.label"/>
					<ui:th text="classGrade.classNo.label"/>
					<ui:th text="classGrade.specialty.label"/>
				</ui:tr>
				</thead>
				<tbody>
					<g:each in="${r}" var="classGradeInstance">
					<ui:tr class="success">
						<td>${fieldValue(bean: classGradeInstance, field: "name")}</td>
...
<ui:form action="importClassGrade">
	<ui:fieldGroup>
		...
		<ui:field name="specialty" label="默认所学专业" hint="省略所学专业的记录，默认为此专业">
			<ui:fieldInput>
				<g:select name="specialty.id" from="${Specialty.list()}" optionKey="id" value="${result?.params?.specialty?.id}" noSelection="['':'']"/>
			</ui:fieldInput>
		</ui:field>
	</ui:fieldGroup>
	<ui:actions>
